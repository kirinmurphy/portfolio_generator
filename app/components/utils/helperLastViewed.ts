...
export function storeCurrentVisitTimestamp (sessionStorage: Storage): void {
  const timestamp = new Date().getTime();
  sessionStorage.setItem(LAST_VIEWED_TIMESTAMP_KEY, JSON.stringify(timestamp));
}

export function wasLastVisitEarlierThan (sessionStorage: Storage, delay: number): boolean {
